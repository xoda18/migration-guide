...
| 7. Expand the source folder | [L45](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S1LaunchAndReadinessTest.kt#L45) | [L69](legacy-robot/src/test/kotlin/com/example/migration/legacy/S1LaunchAndReadinessTest.kt#L69) | The root node carries two pieces of text, the project name and its location on disk, and both sides read them back as one label: `sample-project ~/IdeaProjects`. Driver `expandPath()` takes `fullMatch = false`, so `sample-project` still matches. Legacy `expand(vararg path)` compares labels exactly and has no `fullMatch` variant, so expansion has to go through a double click. |
| 8. Both files are in the tree | [L50](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S1LaunchAndReadinessTest.kt#L50) | [L84](legacy-robot/src/test/kotlin/com/example/migration/legacy/S1LaunchAndReadinessTest.kt#L84) | `findExpandedPath()` compared against null versus `isPathExists()`. Neither of them changes the tree. Driver's `pathExists()` expands first, which would have made this pass whether or not step 7 ran. |
| 9. Open the file by double click | [L55](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S1LaunchAndReadinessTest.kt#L55) | [L89](legacy-robot/src/test/kotlin/com/example/migration/legacy/S1LaunchAndReadinessTest.kt#L89) | Identical, `doubleClickPath()` on both sides. |
| 10. The right file opened | [L59](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S1LaunchAndReadinessTest.kt#L59) | [L92](legacy-robot/src/test/kotlin/com/example/migration/legacy/S1LaunchAndReadinessTest.kt#L92) | Driver asks the editor tabs directly, `editorTabs().isTabOpened(...)` and `codeEditorForFile(...)`. The legacy library has nothing for editor tabs, so the test takes the open editor and asks it which file it shows, which is one more JavaScript call into the IDE. |
