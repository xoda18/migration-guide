...
| 9. Open the file by double click | [L57](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S1LaunchAndReadinessTest.kt#L57) | [L76](remote-robot/src/test/kotlin/com/example/migration/remoterobot/S1LaunchAndReadinessTest.kt#L76) | Identical in shape, `doubleClickPath()` inside a wait on both sides. Either library can drop a click on a busy machine, and neither treats that as a failure: Driver logs "Click was unsuccessful" and carries on. So the wait has to repeat the click rather than watch for a tab that is never coming. |
| 10. The right file opened | [L64](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S1LaunchAndReadinessTest.kt#L64) | [L87](remote-robot/src/test/kotlin/com/example/migration/remoterobot/S1LaunchAndReadinessTest.kt#L87) | The Starter side can ask the editor tabs, and `editorTabs().isTabOpened()` already settled this in step 9, so `codeEditorForFile()` only has to hand over the editor to read. The Remote Robot library has no fixture for editor tabs. So the test takes the open editor with `textEditor()` and reads its `fileName`, and that is one more JavaScript call into the IDE. |
| 11. Collapse and compare | [L68](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S1LaunchAndReadinessTest.kt#L68) | [L93](remote-robot/src/test/kotlin/com/example/migration/remoterobot/S1LaunchAndReadinessTest.kt#L93) | Identical: `collapsePath()`, then a wait for the row count to drop. Neither side settles by itself. Driver's `expandPath()` waits internally and `collapsePath()` does not, so the wait is easy to leave out. |
