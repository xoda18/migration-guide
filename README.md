...
|---|---|---|---|
| 1. Build the context | [L21](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S1LaunchAndReadinessTest.kt#L21) | [L25](legacy-robot/src/test/kotlin/com/example/migration/legacy/S1LaunchAndReadinessTest.kt#L25) | On the Starter side the product, the version, the project and the plugin are values in Kotlin. On the legacy side there is no step 1: it lives in `build.gradle.kts`, outside anything the test can read or assert. |
| 2. Start the IDE, open the project | [L24](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S1LaunchAndReadinessTest.kt#L24) | [L27](legacy-robot/src/test/kotlin/com/example/migration/legacy/S1LaunchAndReadinessTest.kt#L27) | Starter installs the plugin, drops `.idea` and every `.iml`, starts the IDE and shuts it down when the test finishes. Legacy can only open a project, and it opens whatever `.idea` happens to be on disk. |
| 3. Is the plugin loaded | [L27](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S1LaunchAndReadinessTest.kt#L27) | [L30](legacy-robot/src/test/kotlin/com/example/migration/legacy/S1LaunchAndReadinessTest.kt#L30) | `isPluginLoaded()` answers with a typed boolean. The legacy equivalent is a JavaScript string over `PluginManagerCore`, written and maintained in the test. |
| 4. Wait for readiness | [L30](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S1LaunchAndReadinessTest.kt#L30) | [L33](legacy-robot/src/test/kotlin/com/example/migration/legacy/S1LaunchAndReadinessTest.kt#L33) | `waitForIndicators()` covers indexing and every background progress bar. Legacy has only dumb mode, the flag that stays on while the IDE builds its indexes, and it reads that flag through another JavaScript snippet. |
| 5. A project is open | [L34](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S1LaunchAndReadinessTest.kt#L34) | [L38](legacy-robot/src/test/kotlin/com/example/migration/legacy/S1LaunchAndReadinessTest.kt#L38) | Same idea. `project` is a typed handle versus a JS call returning the project name. |
| 6. Open the Project View | [L37](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S1LaunchAndReadinessTest.kt#L37) | [L42](legacy-robot/src/test/kotlin/com/example/migration/legacy/S1LaunchAndReadinessTest.kt#L42) | Driver calls `projectButton.open()`, which invokes the action behind the tool window. The legacy library cannot do that: `CommonSteps.invokeAction()` gives the action no component to read the project from, and it reports nothing when the action then does nothing, so the test clicks the Project button on the left toolbar instead. Driver can also ask the tree whether its nodes are loaded with `waitForNodesLoaded()`. Legacy keeps reading the tree until rows appear. |
| 7. Expand the source folder | [L43](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S1LaunchAndReadinessTest.kt#L43) | [L59](legacy-robot/src/test/kotlin/com/example/migration/legacy/S1LaunchAndReadinessTest.kt#L59) | The root node carries two pieces of text, the project name and its location on disk, and both sides read them back as one label: `sample-project ~/IdeaProjects`. Driver `expandPath()` takes `fullMatch = false`, so `sample-project` still matches. Legacy `expand(vararg path)` compares labels exactly and has no `fullMatch` variant, so expansion has to go through a double click. |
| 8. Both files are in the tree | [L48](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S1LaunchAndReadinessTest.kt#L48) | [L74](legacy-robot/src/test/kotlin/com/example/migration/legacy/S1LaunchAndReadinessTest.kt#L74) | `findExpandedPath()` compared against null versus `isPathExists()`. Neither of them changes the tree. Driver's `pathExists()` expands first, which would have made this pass whether or not step 7 ran. |
| 9. Open the file by double click | [L53](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S1LaunchAndReadinessTest.kt#L53) | [L79](legacy-robot/src/test/kotlin/com/example/migration/legacy/S1LaunchAndReadinessTest.kt#L79) | Identical, `doubleClickPath()` on both sides. |
| 10. The right file opened | [L57](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S1LaunchAndReadinessTest.kt#L57) | [L82](legacy-robot/src/test/kotlin/com/example/migration/legacy/S1LaunchAndReadinessTest.kt#L82) | Driver asks the editor tabs directly, `editorTabs().isTabOpened(...)` and `codeEditorForFile(...)`. The legacy library has nothing for editor tabs, so the test takes the open editor and asks it which file it shows, which is one more JavaScript call into the IDE. |
| 11. Collapse and compare | [L62](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S1LaunchAndReadinessTest.kt#L62) | [L91](legacy-robot/src/test/kotlin/com/example/migration/legacy/S1LaunchAndReadinessTest.kt#L91) | Identical, `collapsePath()` and then reading the expanded paths back. |
...
| Step | Starter and Driver | Legacy robot | What changed |
|---|---|---|---|
| 1. Invoke the action by id | [L23](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S2WaitForEventTest.kt#L23) | [L24](legacy-robot/src/test/kotlin/com/example/migration/legacy/S2WaitForEventTest.kt#L24) | Both avoid keyboard shortcuts, which depend on the OS and the keymap. `invokeAction()`, which takes the context component and the `now` flag, versus a JavaScript string in `pages/IdeaFrame.kt`: `CommonSteps.invokeAction()` hardcodes `now = true`, never opens the Search Everywhere popup, and reports nothing. |
| 2. Type the query | [L28](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S2WaitForEventTest.kt#L28) | [L43](legacy-robot/src/test/kotlin/com/example/migration/legacy/S2WaitForEventTest.kt#L43) | Driver has a typed `searchField` on the popup. The legacy side sets the text on the search field directly, because typing would mean real key presses. Those go wherever the focus is, and the OS decodes them through whatever keyboard layout happens to be active. |
| 3. Wait for the results | [L31](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S2WaitForEventTest.kt#L31) | [L48](legacy-robot/src/test/kotlin/com/example/migration/legacy/S2WaitForEventTest.kt#L48) | `shouldBe()` wraps `waitFor()`, defaults to 15 seconds, and uses its first argument as the text of the timeout error. Legacy has to spell out `waitFor()` and pass a `description` to get the same message. Neither side should ever use `Thread.sleep()`. |
| 4. Click the result by text | [L36](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S2WaitForEventTest.kt#L36) | [L54](legacy-robot/src/test/kotlin/com/example/migration/legacy/S2WaitForEventTest.kt#L54) | Almost identical. By text on both sides, never by position in the list. |
| 5. Close the dialog with a button | [L40](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S2WaitForEventTest.kt#L40) | [L57](legacy-robot/src/test/kotlin/com/example/migration/legacy/S2WaitForEventTest.kt#L57) | Same on both sides, and both had to stop using Escape: the Settings dialog swallows it. |
| 6. No dialogs left | [L46](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S2WaitForEventTest.kt#L46) | [L60](legacy-robot/src/test/kotlin/com/example/migration/legacy/S2WaitForEventTest.kt#L60) | Driver ships `waitForNoOpenedDialogs()`. The legacy side cannot reuse the dialog it just clicked in, so it locates the dialog by title a second time and checks that nothing is found. |
