...
| 4. Wait for readiness | [L32](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S1LaunchAndReadinessTest.kt#L32) | [L30](legacy-robot/src/test/kotlin/com/example/migration/legacy/S1LaunchAndReadinessTest.kt#L30) | `waitForIndicators()` covers indexing, every progress bar in the status bar, and ten quiet seconds in a row, because a gap between two background tasks looks like the end of all of them. The legacy library knows only indexing: `CommonSteps.waitForSmartMode()` waits for dumb mode to end and nothing else. Everything it does not cover falls to the scenario, which is why each one below waits for its own component by hand. |
...
| 6. Open the Project View | [L39](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S1LaunchAndReadinessTest.kt#L39) | [L39](legacy-robot/src/test/kotlin/com/example/migration/legacy/S1LaunchAndReadinessTest.kt#L39) | Driver calls `projectButton.open()`, which invokes the action behind the tool window. The legacy library cannot do that: `CommonSteps.invokeAction()` gives the action no component to read the project from, and it reports nothing when the action then does nothing, so the test clicks the Project button on the left toolbar instead. That button is a toggle while `open()` is not, so the state has to be read before the click, which is only safe because step 4 waited for the IDE to go quiet. Driver can also ask the tree whether its nodes are loaded with `waitForNodesLoaded()`. Legacy keeps reading the tree until rows appear. |
| 7. Expand the source folder | [L45](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S1LaunchAndReadinessTest.kt#L45) | [L58](legacy-robot/src/test/kotlin/com/example/migration/legacy/S1LaunchAndReadinessTest.kt#L58) | The root node carries two pieces of text, the project name and its location on disk, and both sides read them back as one label: `sample-project ~/IdeaProjects`. Driver `expandPath()` takes `fullMatch = false`, so `sample-project` still matches. Legacy `expand(vararg path)` compares labels exactly and has no `fullMatch` variant, so expansion has to go through a double click. |
| 8. Both files are in the tree | [L50](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S1LaunchAndReadinessTest.kt#L50) | [L71](legacy-robot/src/test/kotlin/com/example/migration/legacy/S1LaunchAndReadinessTest.kt#L71) | `findExpandedPath()` compared against null versus `isPathExists()`. Neither of them changes the tree. Driver's `pathExists()` expands first, which would have made this pass whether or not step 7 ran. |
...
| 3. Type the query | [L37](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S2WaitForEventTest.kt#L37) | [L65](legacy-robot/src/test/kotlin/com/example/migration/legacy/S2WaitForEventTest.kt#L65) | Driver has a typed `searchField` on the popup. The legacy side sets the text on the search field directly, because typing would mean real key presses. Those go wherever the focus is, and the OS decodes them through whatever keyboard layout happens to be active. |
| 4. Wait for the results | [L40](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S2WaitForEventTest.kt#L40) | [L70](legacy-robot/src/test/kotlin/com/example/migration/legacy/S2WaitForEventTest.kt#L70) | `shouldBe()` wraps `waitFor()`, defaults to 15 seconds, and uses its first argument as the text of the timeout error. Legacy has to spell out `waitFor()` and pass a `description` to get the same message. Neither side should ever use `Thread.sleep()`. |
| 5. Click the result by text | [L45](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S2WaitForEventTest.kt#L45) | [L76](legacy-robot/src/test/kotlin/com/example/migration/legacy/S2WaitForEventTest.kt#L76) | Almost identical. By text on both sides, never by position in the list. |
| 6. Close the dialog with a button | [L49](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S2WaitForEventTest.kt#L49) | [L79](legacy-robot/src/test/kotlin/com/example/migration/legacy/S2WaitForEventTest.kt#L79) | Same on both sides, and both had to stop using Escape: the Settings dialog swallows it. |
| 7. The dialog really closed | [L55](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S2WaitForEventTest.kt#L55) | [L82](legacy-robot/src/test/kotlin/com/example/migration/legacy/S2WaitForEventTest.kt#L82) | Driver keeps the handle on the dialog it just clicked in, and `shouldBe { notPresent() }` re-resolves that same locator until nothing is found. The legacy side has no handle to reuse, so it spells the title locator out a second time and checks that `findAll()` comes back empty. Driver's generic `waitForNoOpenedDialogs()` is not the answer here: it looks for `//div[@class='MyDialog']`, and the Settings window is not one, so it would report success without checking anything. |
