...
|---|---|---|---|
| 1. Build the context | [L23](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S1LaunchAndReadinessTest.kt#L23) | [L25](legacy-robot/src/test/kotlin/com/example/migration/legacy/S1LaunchAndReadinessTest.kt#L25) | On the Starter side the product, the version, the project and the plugin are values in Kotlin. On the legacy side there is no step 1: it lives in `build.gradle.kts`, outside anything the test can read or assert. |
| 2. Start the IDE, open the project | [L26](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S1LaunchAndReadinessTest.kt#L26) | [L27](legacy-robot/src/test/kotlin/com/example/migration/legacy/S1LaunchAndReadinessTest.kt#L27) | Starter installs the plugin, drops `.idea` and every `.iml`, starts the IDE and shuts it down when the test finishes. Legacy can only open a project, and it opens whatever `.idea` happens to be on disk. |
| 3. Is the plugin loaded | [L29](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S1LaunchAndReadinessTest.kt#L29) | [L30](legacy-robot/src/test/kotlin/com/example/migration/legacy/S1LaunchAndReadinessTest.kt#L30) | `isPluginLoaded()` answers with a typed boolean. The legacy equivalent is a JavaScript string over `PluginManagerCore`, written and maintained in the test. |
| 4. Wait for readiness | [L32](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S1LaunchAndReadinessTest.kt#L32) | [L33](legacy-robot/src/test/kotlin/com/example/migration/legacy/S1LaunchAndReadinessTest.kt#L33) | `waitForIndicators()` covers indexing and every background progress bar. Legacy has only dumb mode, the flag that stays on while the IDE builds its indexes, and it reads that flag through another JavaScript snippet. |
| 5. A project is open | [L36](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S1LaunchAndReadinessTest.kt#L36) | [L38](legacy-robot/src/test/kotlin/com/example/migration/legacy/S1LaunchAndReadinessTest.kt#L38) | Same idea. `project` is a typed handle versus a JS call returning the project name. |
| 6. Open the Project View | [L39](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S1LaunchAndReadinessTest.kt#L39) | [L42](legacy-robot/src/test/kotlin/com/example/migration/legacy/S1LaunchAndReadinessTest.kt#L42) | Driver calls `projectButton.open()`, which invokes the action behind the tool window. The legacy library cannot do that: `CommonSteps.invokeAction()` gives the action no component to read the project from, and it reports nothing when the action then does nothing, so the test clicks the Project button on the left toolbar instead. That button is a toggle while `open()` is not, so the legacy side has to keep clicking until the panel is really open. Driver can also ask the tree whether its nodes are loaded with `waitForNodesLoaded()`. Legacy keeps reading the tree until rows appear. |
| 7. Expand the source folder | [L45](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S1LaunchAndReadinessTest.kt#L45) | [L72](legacy-robot/src/test/kotlin/com/example/migration/legacy/S1LaunchAndReadinessTest.kt#L72) | The root node carries two pieces of text, the project name and its location on disk, and both sides read them back as one label: `sample-project ~/IdeaProjects`. Driver `expandPath()` takes `fullMatch = false`, so `sample-project` still matches. Legacy `expand(vararg path)` compares labels exactly and has no `fullMatch` variant, so expansion has to go through a double click. |
| 8. Both files are in the tree | [L50](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S1LaunchAndReadinessTest.kt#L50) | [L87](legacy-robot/src/test/kotlin/com/example/migration/legacy/S1LaunchAndReadinessTest.kt#L87) | `findExpandedPath()` compared against null versus `isPathExists()`. Neither of them changes the tree. Driver's `pathExists()` expands first, which would have made this pass whether or not step 7 ran. |
| 9. Open the file by double click | [L55](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S1LaunchAndReadinessTest.kt#L55) | [L92](legacy-robot/src/test/kotlin/com/example/migration/legacy/S1LaunchAndReadinessTest.kt#L92) | Identical, `doubleClickPath()` on both sides. |
| 10. The right file opened | [L59](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S1LaunchAndReadinessTest.kt#L59) | [L95](legacy-robot/src/test/kotlin/com/example/migration/legacy/S1LaunchAndReadinessTest.kt#L95) | Driver asks the editor tabs directly, `editorTabs().isTabOpened(...)` and `codeEditorForFile(...)`. The legacy library has nothing for editor tabs, so the test takes the open editor and asks it which file it shows, which is one more JavaScript call into the IDE. |
| 11. Collapse and compare | [L64](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S1LaunchAndReadinessTest.kt#L64) | [L104](legacy-robot/src/test/kotlin/com/example/migration/legacy/S1LaunchAndReadinessTest.kt#L104) | Identical: `collapsePath()`, then a wait for the row count to drop. Neither side settles by itself. Driver's `expandPath()` waits internally and `collapsePath()` does not, so the wait is easy to leave out. |
...
| Step | Starter and Driver | Legacy robot | What changed |
|---|---|---|---|
| 1. Invoke the action by id | [L23](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S2WaitForEventTest.kt#L23) | [L25](legacy-robot/src/test/kotlin/com/example/migration/legacy/S2WaitForEventTest.kt#L25) | Both avoid keyboard shortcuts, which depend on the OS and the keymap. `invokeAction()`, which takes the context component and the `now` flag, versus a JavaScript string in `pages/IdeaFrame.kt`: `CommonSteps.invokeAction()` hardcodes `now = true`, never opens the Search Everywhere popup, and reports nothing. |
| 2. Type the query | [L28](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S2WaitForEventTest.kt#L28) | [L56](legacy-robot/src/test/kotlin/com/example/migration/legacy/S2WaitForEventTest.kt#L56) | Driver has a typed `searchField` on the popup. The legacy side sets the text on the search field directly, because typing would mean real key presses. Those go wherever the focus is, and the OS decodes them through whatever keyboard layout happens to be active. |
| 3. Wait for the results | [L31](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S2WaitForEventTest.kt#L31) | [L61](legacy-robot/src/test/kotlin/com/example/migration/legacy/S2WaitForEventTest.kt#L61) | `shouldBe()` wraps `waitFor()`, defaults to 15 seconds, and uses its first argument as the text of the timeout error. Legacy has to spell out `waitFor()` and pass a `description` to get the same message. Neither side should ever use `Thread.sleep()`. |
| 4. Click the result by text | [L36](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S2WaitForEventTest.kt#L36) | [L67](legacy-robot/src/test/kotlin/com/example/migration/legacy/S2WaitForEventTest.kt#L67) | Almost identical. By text on both sides, never by position in the list. |
| 5. Close the dialog with a button | [L40](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S2WaitForEventTest.kt#L40) | [L70](legacy-robot/src/test/kotlin/com/example/migration/legacy/S2WaitForEventTest.kt#L70) | Same on both sides, and both had to stop using Escape: the Settings dialog swallows it. |
| 6. The dialog really closed | [L46](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S2WaitForEventTest.kt#L46) | [L73](legacy-robot/src/test/kotlin/com/example/migration/legacy/S2WaitForEventTest.kt#L73) | Driver keeps the handle on the dialog it just clicked in, and `shouldBe { notPresent() }` re-resolves that same locator until nothing is found. The legacy side has no handle to reuse, so it spells the title locator out a second time and checks that `findAll()` comes back empty. Driver's generic `waitForNoOpenedDialogs()` is not the answer here: it looks for `//div[@class='MyDialog']`, and the Settings window is not one, so it would report success without checking anything. |
...
| Step | Starter and Driver | Legacy robot | What changed |
|---|---|---|---|
| 1. Open the context menu | [L32](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S3PredicateOnlyTest.kt#L32) | [L36](legacy-robot/src/test/kotlin/com/example/migration/legacy/S3PredicateOnlyTest.kt#L36) | Same right click. Legacy also opens the file, waits for the editor and raises the window; Driver needs none of it. |
| 2. Take the popup | [L35](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S3PredicateOnlyTest.kt#L35) | [L40](legacy-robot/src/test/kotlin/com/example/migration/legacy/S3PredicateOnlyTest.kt#L40) | Driver takes the popup as an object with `popupMenu()` and searches inside it. Legacy has no such object: after the right click the entries are separate top level components, so every later search starts from `RemoteRobot` and goes over all open windows. |
| 3. Describe the entries as two collections | [L38](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S3PredicateOnlyTest.kt#L38) | [L40](legacy-robot/src/test/kotlin/com/example/migration/legacy/S3PredicateOnlyTest.kt#L40) | `xx()` with a typed query versus a raw xpath string. Driver also ships `PopupItemUiComponent`, which reads a label with `getText()`. The library has nothing for a menu entry, so `pages/ActionMenuFixture.kt` wraps one and reads the label through `callJs()`. That page object is why the legacy column points at the same line twice. |
| 4. Poll the submenus with a predicate | [L44](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S3PredicateOnlyTest.kt#L44) | [L46](legacy-robot/src/test/kotlin/com/example/migration/legacy/S3PredicateOnlyTest.kt#L46) | `waitForOne()` versus a retry loop plus `single()`. |
| 5. Click it | [L55](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S3PredicateOnlyTest.kt#L55) | [L55](legacy-robot/src/test/kotlin/com/example/migration/legacy/S3PredicateOnlyTest.kt#L55) | Identical. |
| 6. The submenu really opened | [L60](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S3PredicateOnlyTest.kt#L60) | [L58](legacy-robot/src/test/kotlin/com/example/migration/legacy/S3PredicateOnlyTest.kt#L58) | The check is the same on both sides: after the click there must be more menu entries on screen than before. To get that number the test asks the IDE for the list of entries, and on the legacy side this request throws while the menu is still redrawing, so its wait is `waitForIgnoringError()`. Plain `waitFor()` does not retry after an error, on either side. The Driver wait that retries is `shouldBe()`. |
| 7. Close the submenu | [L65](starter-driver/src/integrationTest/kotlin/com/example/migration/starter/S3PredicateOnlyTest.kt#L65) | [L63](legacy-robot/src/test/kotlin/com/example/migration/legacy/S3PredicateOnlyTest.kt#L63) | Identical. |
